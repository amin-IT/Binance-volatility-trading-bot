...
def msg_discordSnail(msg):

	message = msg + '\n\n'

	mUrl = "https://discord.com/api/webhooks/904566249344868412/HxmaVSgqhMKTqZiqrf_VcC9VqPkNGV_anCwlVSxYIrwXFdrj7S9XmvjjvXxXhHMEHd9L"
			...
			if current_potential_list:
				...
				for i in current_potential_list:
					...
					if get_hist1 >= 0 and get_hist5 >= 0 and get_hist15 >= 0 and get_hist1day >= 0 and get_histbtc >= 0:
						...
						try:
							buyrecomm1d = buy1d.get_analysis().summary['RECOMMENDATION']
							print(f'{coin} {buyrecomm1d} - 1d')
						except Exception as e:
							print("Error - 1d")
							buyrecomm1d = "Error"
						...
						if (buyrecomm1d == "SELL") or (buyrecomm1d == "STRONG_SELL"):
							buynow = False
							print(f'{coin} 1d TF failed')
						...
						try:
							msg_discordSnail(
								f'\n \nMACD Approved Coin {coin} - Potential Profit: {current_potential:.0f}%\nMACD approved coin is effective only as of time of posting\n \n#TradeAtYourOwnRisk \n#CryptoBotAutoPost\n \n---------- \n \n')
							# facebook page
							if Moonings.thisday == datetime.now().day:
								if current_potential >= 60:
									if Moonings.fbcount < 1:
										page_access_token = "EAAFmcNgn0A8BAMGj8Yujo8XiZAn3NRjtzTioDXLujPuOfiCS4vdyyttSrThiDYWnJsPOEnphn3C5y8mZBVhokjao5sbgrJZBU8o0afvb6yKnxtSfa72i58Xp6V3HVDbfhCvUZA7KaegrGrBYxR3jLmh04JeE1vMjAU00YKWgT8ZBafIwLkhoNkqRYqRMuJv0ZD"
										graph = facebook.GraphAPI(page_access_token)
										facebook_page_id = "106867714719862"
										graph.put_object(facebook_page_id, "feed",
														 message=f'MACD Approved Coin {coin} - Potential Profit: {current_potential:.0f}%\nMACD approved coin is effective only as of time of posting\n \nReal-time notification: https://discord.gg/7h2NRUayRw\n \n#TradeAtYourOwnRisk \n#CryptoBotAutoPost')
										Moonings.fbcount += 1
							else:
								Moonings.fbcount = 0
								Moonings.thisday = datetime.now().day
